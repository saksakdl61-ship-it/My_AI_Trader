...
# 결과를 advanced_trader.py가 읽을 수 있는 형식으로 파일에 누적 기록합니다.
...
def generate_and_log_backtest_result():
    ...
    # advanced_trader.py가 읽을 수 있는 형식으로 보고서 작성
    report_entry = f" [종목특성_{stock_type.upper()}] 전략: {strategy_name} -> 총 수익률: {total_profit:.2f}%, 승률: {win_rate:.2f}%, MDD: {mdd:.2f}%\n"
